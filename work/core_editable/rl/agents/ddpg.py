...
class DDPGAgent(Agent):
    ...
    def backward(self, reward, nextstate, info, env, terminal=False):
        ...
        if can_train_either and self.step % self.train_interval == 0:
            ...
            if self.step > self.nb_steps_warmup_critic:
                ...
                if(self.do_PER):
                    """ Prioritised Experience Replay Implementation (update the priorities of transitions with TD errors) """
    ...
    def add_HER(self, env, strategy='future'):
        episode_length = len(self.current_episode_experience)

        if(strategy=='satyen'):
            action_batch = [x[1] for x in self.current_episode_experience]
            nextstate_batch = [x[2] for x in self.current_episode_experience]
            reward_batch = [x[3] for x in self.current_episode_experience]
            terminal_batch = [x[4] for x in self.current_episode_experience]

            nextstate_batch = self.process_state_batch(nextstate_batch)
            terminal_batch = np.array(terminal_batch)
            reward_batch = np.array(reward_batch)
            action_batch = np.array(action_batch)

            assert reward_batch.shape == (episode_length,)
            assert terminal_batch.shape == reward_batch.shape
            assert action_batch.shape == (episode_length, self.nb_actions)

            input_actions = action_batch
            assert input_actions.shape == (episode_length, self.nb_actions)
            if len(self.critic.inputs) >= 3:
                nextstate_batch_with_action = nextstate_batch[:]
            else:
                nextstate_batch_with_action = [nextstate_batch]
            nextstate_batch_with_action.insert(self.critic_action_input_idx, input_actions)
            
            """ CHECK: target_critic or critic? """
            input_q_values = self.critic.predict_on_batch(nextstate_batch_with_action).flatten()
            assert input_q_values.shape == (self.batch_size,)

            """ TODO: Find input TD(1) value functions [check condition for last state] """
        ...
        for t in range(len(self.current_episode_experience)):
            ...
            if(strategy=='satyen'):
                value_sum = np.sum(input_q_values[sample_index:len(self.current_episode_experience)])
                probability_list = [x/value_sum for x in input_q_values[sample_index:len(self.current_episode_experience)]]

            for k in range(self.K):
                if(strategy=='satyen'):
                    new_goal_idx = np.random.choice(range(sample_index,len(self.current_episode_experience)), 1, p=probability_list, replace=False)
                else:
                ...
                """ DEPRECATED WARNING: NonSequentialMemory required """
