        ...
        try:
            while self.step < nb_steps:
                if observation is None:  # start of a new episode  
                    ...
                    callbacks.on_episode_begin(episode)    # (initialise the lists for callback if verbose=2)
                    episode_step = 0
                    episode_reward = 0.
                    ...
                    self.episode_goal = np.array([0,0])    # (sample a goal for the current episode)
                ...
                action = self.forward(observation)             # (stores recent observation and action for memory append)
                if self.processor is not None:
                    action = self.processor.process_action(action)
                reward = 0.
                accumulated_info = {}
                ...
                metrics = self.backward(reward, observation, info, env, terminal=done)          #appended to memory (here observation is now the nextstate)
