...
class AbstractDQNAgent(Agent):
    ...
    def __init__(self, nb_actions, memory, gamma=.99, batch_size=32, nb_steps_warmup=1000,
                 ...
                 delta_range=None, delta_clip=np.inf, custom_model_objects={}, **kwargs):       #changed delta_clip
        ...
        # Related objects.
        self.memory = memory
        self.current_episode_experience = []    # (stores current episode experience as [s',a,r,is_terminal])
        self.K = 4  #(HER parameter)
...
class DQNAgent(AbstractDQNAgent):
    ...
    def add_HER(self,env):
        for t in range(len(self.current_episode_experience)):
            for k in range(self.K):
                new_goal_idx = np.random.randint(t,len(self.current_episode_experience))
                new_goal = self.current_episode_experience[new_goal_idx][2][3:6]    # randomly sampled substitute goal from states seen after the current transition
                
                # update the original transition
                curr_observation, curr_action, her_goal_observation, curr_reward, curr_terminal, info = self.current_episode_experience[t]
                her_achieved_goal = deepcopy(her_goal_observation[3:6])
                her_goal_observation[-3:] = new_goal 
                her_reward = env.compute_reward(her_achieved_goal, new_goal, info)
                self.memory.append(curr_observation, curr_action, her_goal_observation, her_reward, curr_terminal)
