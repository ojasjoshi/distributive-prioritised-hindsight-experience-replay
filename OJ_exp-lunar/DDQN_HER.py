...
class Replay_Memory():
	...
	def __init__(self, memory_size=100000, burn_in=20000):
...
class DQN_Agent():
	...
	def train(self):
		...
		for e in range(self.num_episodes):																			#uncomment for mountaincar
			...
			while(True): 																							#uncomment for mountaincar
				...
				temp_state = curr_state
				temp_nextstate = nextstate
				#appending current
				self.replay_mem.append([temp_state.reshape(1,temp_state.shape[0]),curr_action,reward/200,temp_nextstate.reshape(1,temp_nextstate.shape[0]),is_terminal])			
				if(is_terminal):
					break		
	...
	def burn_in_memory(self):
		...
		while(curr_mem_size<self.replay_mem.burn_in):
			...
			episode_experience.append([state,action,reward/200,nextstate,self.main_goal,is_terminal])
			if(is_terminal == True):
